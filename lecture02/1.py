...
if len(number) == 2:
    #первый десяток особенный, потому нужно расписать его целиком + есть проблема,
    #ибо result перезаписывается 2 раза в случае работы с первым десятком (ноль -> десять)
    if number == '10':
        result = 'десять'
    if number == '11':
        result = 'одиннадцать'
